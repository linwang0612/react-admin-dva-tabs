...
class AddTenantFactoryComp extends PureComponent {
  constructor(props) {
    super(props);
  }
  ...
  componentDidMount() {
    ...
    detail.provinceCode && this.props.dispatch({ type: 'tenantFactory/setSelectVal', provinceCode: detail.provinceCode, cityCode: detail.cityCode, countyCode: detail.countyCode })
  }
  ...
  onSubmit = (e) => {
    ...
    this.props.form.validateFieldsAndScroll((err, values) => {
      ...
      const { provinceCode, cityCode, countyCode } = this.props.tenantFactory;
      delete values.area
      values = Object.assign({}, this.props.detail, values, { provinceCode: provinceCode }, { cityCode: cityCode }, { countyCode: countyCode })
      this.props.onSubmit(values)
    })
  }
  addrOptionsChange = options => {
    this.props.dispatch({ type: 'tenantFactory/setSelectVal', provinceCode: options.length > 0 && options[0].value, cityCode: options.length > 1 && options[1].value, countyCode: options.length > 2 && options[2].value })
  }
  render() {
    const { detail } = this.props;
...
export default Form.create()(AddTenantFactoryComp)
